...
Resolves three per-rank inputs before `MooncakeDistributedStore.setup()`:
  * the RDMA NIC name to use,
  * the address this rank announces as ``local_hostname``,
  * the optional ``preferred_segment`` to pin replicas to.

The operator must set ``device_name`` in ``mooncake_config.json`` (either a
single name or a comma-separated CSV indexed by physical GPU). If nothing is
set we log a warning and hand off to Mooncake's C++ auto-selection — there
is no Python-side discovery fallback (matches the vllm-ascend approach;
see ``docs/design/rdma-utils-simplification.md``).
"""
