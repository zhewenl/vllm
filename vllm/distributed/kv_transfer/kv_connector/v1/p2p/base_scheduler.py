...
"""Base scheduler-side logic for the P2P connector."""
...
from vllm.distributed.kv_transfer.kv_connector.v1.p2p.utils import zmq_ctx
...
class P2pBaseConnectorScheduler:
    ...
    def __init__(
        ...
        self.side_channel_host = vllm_config.kv_transfer_config.get_from_extra_config(
        ...
        logger.info("Initializing P2P Scheduler %s", engine_id)
        if vllm_config.scheduler_config.disable_hybrid_kv_cache_manager:
            logger.info("Hybrid Memory Allocator is enabled with P2P")

        # Background thread for handling new handshake requests.
        self._p2p_handshake_listener_t: threading.Thread | None = None
    ...
    def shutdown(self):
        ...
        if self._p2p_handshake_listener_t is not None:
            self._p2p_handshake_listener_t.join()
            self._p2p_handshake_listener_t = None
    ...
    def set_xfer_handshake_metadata(
        ...
        for (pp_rank, tp_rank), rank_metadata in metadata.items():
            if not isinstance(rank_metadata, P2pHandshakePayload):
                raise ValueError(
                    "P2pConnectorScheduler expects P2pHandshakePayload for "
                    "handshake metadata."
                )
            encoded_data[(pp_rank, tp_rank)] = encoder.encode(rank_metadata)
            logger.debug(
                "PP rank %d, TP rank %d: encoded P2pHandshakePayload size: %s bytes",
                pp_rank,
                tp_rank,
        ...
        if self._p2p_handshake_listener_t is None:
            ready_event = threading.Event()
            self._p2p_handshake_listener_t = threading.Thread(
                target=self._p2p_handshake_listener,
                args=(
                    encoded_data,
                    ready_event,
                    self._stop_event,
                ...
                daemon=True,
                name="p2p_handshake_listener",
            )
            self._p2p_handshake_listener_t.start()
            ready_event.wait()  # Wait for listener ZMQ socket to be ready.

    @staticmethod
    def _p2p_handshake_listener(
        ...
        """Background thread for getting new P2P handshakes."""
