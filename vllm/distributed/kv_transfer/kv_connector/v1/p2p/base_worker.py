...
"""Base worker-side logic for the P2P connector."""
...
def _tensor_byte_span_end(cache: torch.Tensor) -> int:
    ...
    if any(stride < 0 for stride in cache.stride()):
        raise ValueError("P2P cache views must have nonnegative strides")
...
def _uses_dense_virtual_transfer_pages(
    ...
    """Return whether a compressed kernel view can be split into P2P pages."""
    if not (
...
class P2pBaseConnectorWorker:
    ...
    def _compute_desc_ids(
        ...
        """Compute P2P descriptor IDs for given block IDs."""
        num_ssm_regions = 0
        if self._has_mamba:
            assert self._conv_decomp is not None
            # P2P regions per SSM layer = conv sub-projections + 1 SSM temporal
    ...
    def __init__(
        ...
        if vllm_config.kv_transfer_config is None:
            raise ValueError("kv_transfer_config must be set for P2pConnector")
        ...
        if self._is_csa_linear:
            ...
            if len(ple_groups) != 1 or len(ple_groups[0][1].layer_names) != 1:
                raise ValueError("CSA-linear P2P requires exactly one PLE cache owner.")
        ...
        # KV Caches and p2p tracking data.
        self.device_type = current_platform.device_type
        self.kv_buffer_device: str = vllm_config.kv_transfer_config.kv_buffer_device
        self.transport = create_transport(
            vllm_config, "READ" if self._TRANSFER_MODE == "pull" else "WRITE"
        )
        self.device_kv_caches: dict[str, torch.Tensor] = {}

        # cpu kv buffer for xfer
        # used when device memory can not be registered under p2p
        ...
        # Number of P2P regions. Currently one region per cache
        ...
        if self.pp_size > 1 and self._is_hma_required:
            raise NotImplementedError(
                "P2pPushConnector does not support pipeline_parallel_size > 1 "
        ...
        if vllm_config.kv_transfer_config.kv_role == "kv_consumer" and self.pp_size > 1:
            raise NotImplementedError(
                "P2pPushConnector consumer (decode) does not support "
                "pipeline_parallel_size > 1."
            )
        # Keep heartbeat handshakes to a PP-sharded producer notif-only.
        self._hb_handshake_notif_only = False

        # p2p_prepped_dlist_handle.
        self.src_xfer_handles_by_block_size: dict[int, TransferHandle] = {}
        ...
        self.src_xfer_handles_by_tp_ratio: dict[
            tuple[int, int], list[TransferHandle]
        ] = {}
        # Map of engine_id -> {tp_rank: prepared remote descriptors}.
        self.dst_xfer_side_handles = defaultdict[EngineId, dict[int, TransferHandle]](
            dict
        )
        ...
        # Invalid blocks from failed P2P operations (thread-safe queue of block ids)
        ...
        # Handshake metadata of this worker for P2P transfers.
        self.xfer_handshake_metadata: P2pHandshakePayload | None = None
        # Background thread for initializing new P2P handshakes.
        self._handshake_initiation_executor = ThreadPoolExecutor(
            # P2P is not guaranteed to be thread-safe, limit 1 worker.
            max_workers=1,
            thread_name_prefix="vllm-p2p-handshake-initiator",
        )
        ...
        self.xfer_stats = P2pKVConnectorStats()
    ...
    def _validate_remote_parallel_config(
        self, agent_metadata: P2pAgentMetadata
    ) -> None:
        ...
        if (local_pcp_size > 1 and remote_dcp_size > 1) or (
            ...
            raise NotImplementedError(
                "P2pConnector PCP requires decode_context_parallel_size=1 "
                "on both instances. "
    ...
    def _validate_csa_linear_tp_layout(self, remote_tp_size: int) -> None:
        ...
        if (self.world_size < total_kv_heads) != (remote_tp_size < total_kv_heads):
            raise ValueError(
                "CSA-linear P2P requires both endpoints on the same side of "
    ...
    def _p2p_handshake(
        ...
        """Do a P2P handshake with a remote instance."""
        if self._is_csa_linear:
        ...
        with zmq_ctx(zmq.REQ, path) as sock:
            for remote_pp_rank, remote_rank in itertools.product(
                ...
                handshake_decoder = msgspec.msgpack.Decoder(P2pHandshakePayload)
                try:
                    handshake_payload = handshake_decoder.decode(handshake_bytes)
                except (msgspec.DecodeError, msgspec.ValidationError) as e:
                    raise RuntimeError(
                        f"Failed to decode P2pHandshakePayload. This likely indicates "
                        f"an incompatibility between connector version. Error: {e}"
                    ) from e

                got_metadata_time = time.perf_counter()
                logger.debug(
                    "P2P handshake: get metadata took: %s",
                ...
                if (
                    ...
                    raise RuntimeError(
                        f"P2P compatibility hash mismatch. "
                ...
                logger.info(
                    "P2P compatibility check passed (hash: %s)",
                    handshake_payload.compatibility_hash,
                )

                # Decode agent metadata
                metadata_decoder = msgspec.msgpack.Decoder(P2pAgentMetadata)
                try:
                    ...
                    raise RuntimeError(
                        f"Failed to decode P2pAgentMetadata. Error: {e}"
                    ) from e
                ...
                if metadata.engine_id != expected_engine_id:
                    raise RuntimeError(
                        f"Remote P2P agent engine ID mismatch. "
                ...
                logger.debug(
                    "P2P handshake: add agent took: %s (notif_agents_only=%s)",
    ...
    def _add_notif_only_remote_agent(
        self, metadata: P2pAgentMetadata, remote_tp_size: int, remote_dcp_size: int = 1
    ) -> str:
        ...
        return self.transport.connect_peer(metadata.agent_metadata)

    def initialize_host_xfer_buffer(self, kv_caches: dict[str, torch.Tensor]) -> None:
        """
        Initialize transfer buffer in CPU mem for accelerators
        NOT directly supported by P2P (e.g., tpu)
        """
        ...
        try:
                ...
                if not self.use_mla:
                    ...
                    if self.kv_cache_layout == "LBNHC":
                        if self.kv_transfer_config.enable_permute_local_kv:
                            ...
                            # so raw P2P transfers see NHD physical strides.
            ...
            logger.error("P2PConnectorWorker gets %s.", e)
            raise
    ...
    def _log_failure(
        ...
        **extra_context,
        ...
        logger.error(
            "P2P transfer failure: %s | Context: %s",
            failure_type,
            context,
    ...
    def _background_p2p_handshake(
        self, req_id: str, remote_engine_id: EngineId, meta: ReqMeta
    ):
        # Do P2P handshake in background and add to _ready_requests when done.
    ...
    def register_kv_caches(self, kv_caches: dict[str, torch.Tensor]):
        """Register the KV Cache data in p2p."""
        ...
        self.compat_hash = compute_p2p_compatibility_hash(
            self.vllm_config,
            self.backend_name,
        ...
        if self._is_csa_linear and self.use_host_buffer:
            raise NotImplementedError(
                "P2P host staging does not preserve CSA-linear shared tensors."
            )
        ...
        # single P2P region whose block transfers as one unit. Mamba layers instead
        ...
        for layer_name, cache in xfer_buffers.items():
            ...
            if isinstance(layer_spec, MambaSpec):
                ...
                if virtual_transfer_pages:
                    # A compressed kernel row can contain multiple P2P transfer pages.
                    region_specs = [
                    ...
                    if cache.nbytes % segment_bytes != 0:
                        raise AssertionError(
                            "KV cache view cannot be partitioned into P2P regions: "
        ...
        descs = self.transport.register_memory(caches_data)
        ...
        # Register local/src descr for P2P xfer.
        self.src_xfer_handles_by_block_size[self.block_size], self.src_blocks_data = (
            self.register_local_xfer_handler(self.block_size)
        )

        # After KV Caches registered, listen for new connections.
        agent_metadata = P2pAgentMetadata(
            engine_id=self.engine_id,
            agent_metadata=self.transport.export_peer(),
        ...
        self.xfer_handshake_metadata = P2pHandshakePayload(
    ...
    def _build_mamba_remote(
        self,
        p2p_agent_meta: P2pAgentMetadata,
        tp_ratio: int,
        ...
        assert p2p_agent_meta.kv_caches_base_addr, (
        ...
        region_indices = self._ssm_region_indices or range(
            len(p2p_agent_meta.kv_caches_base_addr)
        )
        for i in region_indices:
            base_addr = p2p_agent_meta.kv_caches_base_addr[i]
            block_stride = p2p_agent_meta.block_strides[i] * remote_physical_per_logical
            blk_addrs = base_addr + block_arange * block_stride
            for off, sz in conv_offsets:
                parts.append(self._stack_descs(blk_addrs + off, sz, device_id))
        ...
        if (region_index := self._ple_region_index) is not None:
            ...
            remote_block_len = (
                p2p_agent_meta.block_lens[region_index] * remote_physical_per_logical
            )
            ...
            remote_block_stride = (
                p2p_agent_meta.block_strides[region_index] * remote_physical_per_logical
            )
            block_addrs = (
                p2p_agent_meta.kv_caches_base_addr[region_index]
    ...
    def _build_fa_remote(
        ...
        num_blocks = p2p_agent_meta.num_blocks
        device_id = p2p_agent_meta.device_id
        block_arange = np.arange(num_blocks, dtype=np.uint64)
        parts: list[np.ndarray] = []
        for i, base_addr in enumerate(p2p_agent_meta.kv_caches_base_addr):
            replicated = self._is_region_replicated(i)
            ...
            page_size = p2p_agent_meta.block_strides[i]
    ...
    def register_local_xfer_handler(
        self,
        block_size: int,
    ) -> tuple[TransferHandle, np.ndarray]:
        """
        ...
        return self.transport.prepare_descriptors(None, blocks_data), blocks_data
    ...
    def add_remote_agent(
        ...
        """
        Add the remote P2P agent and prepare the descriptors for reading cache
        blocks from remote.
        ...
        """  # noqa: E501
        engine_id = p2p_agent_meta.engine_id
        ...
        if (
            self.pp_size > 1
            and len(p2p_agent_meta.kv_caches_base_addr) > num_local_regions
        ):
            ...
            assert len(p2p_agent_meta.kv_caches_base_addr) >= end
            p2p_agent_meta.kv_caches_base_addr = p2p_agent_meta.kv_caches_base_addr[
                start:end
            ]
            p2p_agent_meta.block_lens = p2p_agent_meta.block_lens[start:end]
            p2p_agent_meta.block_strides = p2p_agent_meta.block_strides[start:end]

        ### Register remote engine in TransferTopology (idempotent).
        assert self.transfer_topo is not None
        transfer_topo = self.transfer_topo
        physical_blocks_per_logical = (
            p2p_agent_meta.physical_blocks_per_logical_kv_block
        )
        transfer_info = EngineTransferInfo(
            remote_tp_size=remote_tp_size,
            remote_block_size=p2p_agent_meta.block_size,
            remote_block_len=p2p_agent_meta.block_lens[0],
        ...
        remote_agent_name = self.transport.connect_peer(p2p_agent_meta.agent_metadata)
        ...
        block_size_ratio = transfer_topo.block_size_ratio(p2p_agent_meta.block_size)

        if engine_id not in self.dst_num_blocks:
            self.dst_num_blocks[engine_id] = p2p_agent_meta.num_blocks

        # Keep track of remote agent kv caches base addresses.
        self.kv_caches_base_addr[engine_id][remote_tp_rank] = (
            p2p_agent_meta.kv_caches_base_addr
        )
        self._validate_remote_agent_handshake(
            p2p_agent_meta, remote_tp_size, remote_dcp_size
        )
        ...
        if self._needs_split_local_xfer_handles(tp_ratio, plan) and (
            ...
            for handle_data in self._build_local_splits_from_plan(
                ...
                handle = self.transport.prepare_descriptors(None, handle_data)
        ...
        if self._has_mamba:
            ...
            mamba = self._build_mamba_remote(p2p_agent_meta, tp_ratio, transfer_info)
            blocks_data = np.concatenate([blocks_data, mamba])

        # Register with P2P.
        self.dst_xfer_side_handles[engine_id][remote_tp_rank] = (
            self.transport.prepare_descriptors(remote_agent_name, blocks_data)
        )
    ...
    def _validate_remote_agent_handshake(
        ...
        """
        remote_engine_id = p2p_agent_meta.engine_id
        ...
        if not self.use_mla and p2p_agent_meta.kv_cache_layout != kv_cache_layout:
            if (
                self.kv_transfer_config.enable_permute_local_kv
                and p2p_agent_meta.kv_cache_layout == "LBHNC"
            ):
                logger.info(
        ...
        if (
            p2p_agent_meta.attn_backend_name != self.backend_name
        ...
        if self._has_mamba and self.use_mla:
            ...
            assert self.block_len_per_layer == [
                block_len * block_size_ratio for block_len in p2p_agent_meta.block_lens
            ], (
                ...
                f"remote={p2p_agent_meta.block_lens}."
            )
        elif not self._has_mamba:
            assert len(self.block_len_per_layer) == len(p2p_agent_meta.block_lens), (
            ...
            for i, local_len in enumerate(self.block_len_per_layer):
                ...
                remote_len = p2p_agent_meta.block_lens[i]
                if replicated:
        ...
        assert self.dst_num_blocks[remote_engine_id] == p2p_agent_meta.num_blocks
        # Same number of regions/~layers.
        assert len(p2p_agent_meta.kv_caches_base_addr) == len(self.block_len_per_layer)
    ...
    def save_kv_to_host(self, metadata: P2pConnectorMetadata):
    ...
    def get_finished(self) -> tuple[set[str], set[str]]:
        ...
        """
        assert self.transfer_topo is not None
        done_sending = self._get_notifications()
    ...
    def _get_notifications(self) -> set[str]:
    ...
    def _pop_done_transfers(
        ...
        for req_id, handles in list(transfers.items()):
            ...
            for handle in handles:
                try:
                    xfer_state = self.transport.poll(handle)
                    if xfer_state == TransferState.DONE:
                        # Get telemetry from P2P
                        res = self.transport.telemetry(handle)
                        self.xfer_stats.record_transfer(res)
                        self.transport.release_transfer(handle)
                    elif xfer_state == TransferState.PENDING:
    ...
    def _handle_failed_transfer(self, req_id: str, handle: TransferHandle | None):
        """
    ...
    def _send_heartbeats(self, metadata: P2pConnectorMetadata) -> None:
        """
        ...
        for engine_id, hb_info in metadata.heartbeat_by_engine.items():
            ...
            for agent_name in self._remote_agents[engine_id].values():
                try:
                    self.transport.send_notification(agent_name, notif_msg=hb_msg)
                except Exception:
                    logger.debug(
    ...
    def get_block_ids_with_load_errors(self) -> set[int]:
        ...
        to be retried after a P2P transfer failure.
        """
    ...
    def _cleanup_remote_engine(
        ...
        Releases P2P resources (dlist handles, remote agents) and clears
        ...
        for handle in self.dst_xfer_side_handles.pop(engine_id, {}).values():
            self.transport.release_descriptors(handle)
        for agent_name in self._remote_agents.pop(engine_id).values():
            self.transport.disconnect_peer(agent_name)
    ...
    def shutdown(self):
        ...
        for desc in self._registered_descs:
            self.transport.unregister_memory(desc)
