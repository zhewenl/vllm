...
"""Metadata dataclasses and helpers for the P2P connector."""
...
#   3: Add physical_blocks_per_logical_kv_block to P2pAgentMetadata
...
#   7: Include P2P transfer mode (push vs pull) in the compatibility hash
#   8: Add dcp_size and pcp_size to P2pAgentMetadata
#   9: Add block_strides
#  10: Add dense virtual transfer pages for compressed MLA caches
#
P2P_CONNECTOR_VERSION: int = 10


@dataclass
class P2pAgentMetadata:
    engine_id: str
...
@dataclass
class P2pHandshakePayload(KVConnectorHandshakeMetadata):
    """
    Wrapper for P2P handshake sent over the wire.

    Enables two-phase decoding for graceful compatibility checking:
    1. Decode P2pHandshakePayload to get compatibility_hash
    2. Compute local hash and compare
    3. Only if hashes match, decode agent_metadata_bytes

    This prevents decoder errors when P2pAgentMetadata schema is
    incompatible, allowing graceful failure with clear error message.
    """

    compatibility_hash: str
    agent_metadata_bytes: bytes  # P2pAgentMetadata encoded


def _get_speculative_compatibility_factors(
    vllm_config: VllmConfig,
) -> dict[str, Any] | None:
    """Return P2P compatibility factors for hidden-state-based speculators."""
...
def compute_p2p_compatibility_hash(
    ...
    """
    Compute compatibility hash for P2P KV transfer.

    Hash only the factors that affect whether two P2P instances can
    successfully transfer KV cache data.

    Factors included:
    - vLLM version and P2P connector version
    ...
    factors = {
        ...
        "nixl_connector_version": P2P_CONNECTOR_VERSION,
    ...
    transfer_engine = get_transfer_engine(vllm_config)
    if transfer_engine != "nixl":
        factors["transfer_engine"] = transfer_engine
        factors["transport_protocol_version"] = 1
    compat_hash = hash_factors(factors)
    logger.debug(
        "P2P compatibility hash: %s (model=%s, dtype=%s, num_kv_heads=%d, "
...
class ReqMeta:
    ...
    # Remote block size, discovered during P2P handshake (push mode).
    remote_block_size: int | None = None
    # Remote producer pipeline-parallel size (push mode, D side).
    pp_size: int = 1


class P2pConnectorMetadata(KVConnectorMetadata):
    def __init__(self):
        ...
        # P workers via P2P notification on this step.
