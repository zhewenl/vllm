...
"""Pull-specific (READ) worker-side logic for the P2P connector."""
...
class P2pPullConnectorWorker(P2pBaseConnectorWorker):
    ...
    def start_load_kv(self, metadata: P2pConnectorMetadata):
        """
        Start loading by triggering non-blocking p2p_xfer.
        ...
        for req_id, meta in metadata.reqs_to_recv.items():
            ...
            if remote_engine_id not in self._remote_agents:
                ...
                with self._handshake_lock:
                    if remote_engine_id not in self._remote_agents:
                        self._background_p2p_handshake(req_id, remote_engine_id, meta)
                        continue
        ...
        if self.use_mla and tp_ratio < 0 and len(read_specs) == 1:
            ...
            for rank_to_notify, agent in remote_agents.items():
                if rank_to_notify != (0, read_specs[0].remote_rank):
                    self.transport.send_notification(agent, notif_msg=notif_id)
    ...
    def _read_blocks(
        ...
        if not any(len(group) > 0 for group in local_block_ids):
            ...
            try:
                self.transport.send_notification(agent_name, notif_msg=notif_id)
