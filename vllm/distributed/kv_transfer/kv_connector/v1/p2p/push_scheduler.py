...
"""Push-specific scheduler-side logic for the P2P connector.
...
* D side (decode): on ``update_state_after_alloc``, stash registration data
  ...
  ``meta.push_registrations`` next step and sends a P2P notification to the
...
* P side (prefill): on ``request_finished``, stash the finished block IDs
  ...
  it already received via P2P notifications.
...
class P2pPushConnectorScheduler(P2pBaseConnectorScheduler):
    """Push-specific scheduler logic (WRITE-based KV transfer).

    All P2P communication is deferred to the worker level via P2P
    ...
    def __init__(
        ...
        if self.is_bidirectional_kv_xfer_enabled:
            raise NotImplementedError(
                "Bidirectional KV transfer is not supported for P2P push connector."
            )
    ...
    def get_num_new_matched_tokens(
        ...
        logger.debug(
            "P2pPushConnector get_num_new_matched_tokens: "
    ...
    def update_state_after_alloc(
        ...
        to P via P2P notification (deferred to ``build_connector_meta``).
        """
        params = request.kv_transfer_params
        logger.debug(
            "P2pPushConnector update_state_after_alloc: "
        ...
        # learned by P over the P2P handshake at WRITE time.
    ...
    def request_finished(
        ...
        params = request.kv_transfer_params
        logger.debug(
            "P2pPushConnector request_finished(%s), request_status=%s, "
        ...
        if delay_free_blocks:
            logger.debug(
                "P2pPushConnector request_finished(%s) waiting for %d seconds "
            ...
            # registrations (via P2P notifications).
    ...
    def build_connector_meta(
        ...
        assert isinstance(meta, P2pConnectorMetadata)
        ...
        for rid in expired:
            ...
            logger.warning(
                "P2pPushConnector: registration for request %s timed out "
        ...
        # any D registrations they have received via P2P notifications.
