...
"""Push-specific (WRITE) worker-side logic for the P2P connector.

A dedicated ``p2p-push-writer`` thread owns all push-related P2P ops:
calls ``get_notifications`` (routing PUSH_REG internally; HB / completion
notifs are forwarded to the engine main thread), sends PUSH_REG via
``send_notification``, matches D registrations with P finished blocks, and
issues WRITE transfers via ``create_transfer`` / ``submit``.
...
(so each engine step gives the writer a chance to drain P2P notifs);
...
from vllm.distributed.kv_transfer.kv_connector.v1.p2p.utils import get_base_request_id
...
class P2pPushConnectorWorker(P2pBaseConnectorWorker):
    """Push-specific (WRITE) worker logic. See module docstring."""

    # Distinguishes push from pull in the P2P compatibility hash.
    ...
    def __init__(
        ...
        # P-side: D registrations received via P2P notification that have
    ...
    def register_kv_caches(self, kv_caches: dict[str, "torch.Tensor"]):
        ...
        if self._push_writer_thread is None:
            self._push_writer_thread = threading.Thread(
                ...
                daemon=True,
                name="p2p-push-writer",
            )
            self._push_writer_thread.start()
            logger.info("p2p-push-writer thread started (rank=%d)", self.tp_rank)

    def shutdown(self):
        if not hasattr(self, "_push_writer_stop"):
            super().shutdown()
            return
    ...
    def start_load_kv(self, metadata: P2pConnectorMetadata):
        """Pre-process metadata; defer P2P ops to the writer thread."""
        if self.pcp_rank > 0:
            return
        ...
        # --- D-side: registrations to send to P via P2P ---
    ...
    def _push_writer_loop(self) -> None:
        ...
        while not self._push_writer_stop.is_set():
            try:
                ...
                # 4. P2P notifs: route PUSH_REG; forward the rest.
                for notifs in self.transport.get_notifications().values():
                    for notif in notifs:
                ...
                logger.exception("p2p-push-writer error; continuing")
    ...
    def _send_registration_to_p(
        ...
        """Handshake (if needed) then send PUSH_REG. ``send_notification`` always
    ...
    def _do_send_reg_notif(self, req_id: str, reg_data: dict[str, Any]) -> None:
        ...
        for rank, agent_name in agents.items():
            try:
                self.transport.send_notification(agent_name, notif_msg=notif_msg)
            except Exception as e:
                self._log_failure(
    ...
    def _do_start_push_kv(
        ...
        if elapsed_ms > 200.0:
            logger.warning(
                "_do_start_push_kv for %s took %.1fms (slow P2P submission)",
                request_id,
                elapsed_ms,
            )
    ...
    def _xfer_blocks_for_req(self, req_id: str, meta: ReqMeta):
        ...
        # P2P handshake. ``meta`` is freshly built by
    ...
    def _xfer_blocks(
        ...
        local_xfer_side_handle: TransferHandle,
        remote_xfer_side_handle: TransferHandle,
    ) -> TransferHandle | None:
        ...
        handle = None
        try:
            handle = self.transport.create_transfer(
                "WRITE",
    ...
    def _get_notifications(self) -> set[str]:
        ...
        The writer owns ``transport.get_notifications`` for push; PUSH_REG
        ...
        while True:
            ...
            if req_id not in self._reqs_to_send and req_id not in self._reqs_to_process:
                if (meta := self._recving_metadata.get(req_id)) is not None:
                    ...
                    # P drove the transfer (we own no P2P handle), so
    ...
    def get_finished(self) -> tuple[set[str], set[str]]:
        ...
        # so it gets a chance to drain P2P notifs (heartbeats, completion
